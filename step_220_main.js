...
scene.background = new THREE.Color('#222d26');
scene.fog = new THREE.Fog('#222d26', 50, 110);
...
let hero = pawn(0x819ec6, 'TransKnight', false);
let enemy = pawn(0xa76543, 'Javali', true);
...
async function selectCharacter(kind) {
  if (!characterCache.has(kind)) {
    log(`Carregando personagem ${kind}…`);
    if (kind === 'theodore') {
  ...
  state.hero = kind === 'foema' ? 'Foema' : 'TransKnight';
  save();
  updateUI();
  log(`Classe ativa: ${state.hero}.`);
}
...
function showTooltip(item, e) {
  ...
  statsHtml += `<span>Preço de Venda: ${def.price}g</span>`;
...
function renderInventory() {
  ...
  $('menu-inv-badge').textContent = totalItems;
}
...
function updateUI() {
  ...
  $('upgrade').textContent = `Refinar Arma (+${state.weapon || 0}) [${price(state)}g]`;
...
document.querySelectorAll('.btn-add-stat').forEach((btn) => {
  btn.onclick = () => {
    ...
    if (allocateStat(state, stat)) {
      ...
      save();
      updateUI();
      log(`+1 ponto alocado em ${stat.toUpperCase()}.`);
    } else {
      log('Sem pontos de atributo para distribuir.');
    }
  };
});
...
$('upgrade').onclick = () => {
  if (upgrade(state)) {
    sfx.levelUp();
    showFloatingText('REFINE +1!', 'gold');
    log('Arma refinada com sucesso! Dano aumentado.');
    save();
    updateUI();
  }
};
...
function closeAllWindows() {
  document.querySelectorAll('.wyd-panel-window').forEach((w) => w.classList.add('is-hidden'));
  sfx.toggle();
}
...
$('btn-menu-char').onclick = () => toggleWindow('character-window');
$('btn-menu-inv').onclick = () => toggleWindow('inventory-window');
$('btn-menu-skills').onclick = () => toggleWindow('skills-window');
$('btn-menu-quests').onclick = () => toggleWindow('quests-window');
$('btn-menu-lobby').onclick = () => {
  $('lobby').style.display = 'flex';
  closeAllWindows();
};

document.querySelectorAll('.wyd-close-btn').forEach((btn) => {
  btn.onclick = () => {
...
addEventListener('keydown', (e) => {
  if (e.code === 'KeyC') toggleWindow('character-window');
  if (e.code === 'KeyV' || e.code === 'KeyI') toggleWindow('inventory-window');
  if (e.code === 'KeyK') toggleWindow('skills-window');
  if (e.code === 'KeyQ') toggleWindow('quests-window');
...
function hit() {
  ...
  $('target-box').classList.remove('is-hidden');
  lastHit = now;
  sfx.swing();
...
function checkEnemyStatus() {
  if (enemyCurrentHp <= 0) {
    ...
    if (state.level > prevLvl) {
      ...
      log(`Parabéns! Você alcançou o Nível ${state.level}! (+5 Pontos de Atributo)`);
    }
...
$('hit').onclick = hit;
$('close-target').onclick = () => $('target-box').classList.add('is-hidden');
...
$('auto').onclick = () => {
  ...
  $('target-box').classList.remove('is-hidden');
  log(auto ? 'Modo de caça automática ATIVADO.' : 'Modo de caça automática PAUSADO.');
};

// Lobby Slots Selection
document.querySelectorAll('#lobby .slot').forEach((slot) => {
  slot.onclick = () => {
    document.querySelectorAll('#lobby .slot').forEach((s) => s.classList.remove('selected'));
...
$('enter-world').onclick = async () => {
  preloadSounds();
  sfx.click();
  const selectedSlot = document.querySelector('#lobby .slot.selected');
  const chosenClass = selectedSlot ? selectedSlot.dataset.class : 'transknight';
  $('lobby').style.display = 'none';
  await selectCharacter(chosenClass);
...
try {
  ...
  const initialHero = localStorage.getItem(characterKey) || 'transknight';
  try {
    await selectCharacter(initialHero);
  } catch {
    await selectCharacter('transknight');
  }
} catch (e) {
...
renderer.setAnimationLoop((time) => {
  ...
  // Atualizar coordenadas e minimap blip
  const coordX = Math.round(2100 + hero.position.x);
  const coordY = Math.round(2090 + hero.position.z);
  $('player-coords').textContent = `X: ${coordX}, Y: ${coordY}`;
  const blip = $('player-minimap-blip');
  if (blip) {
    blip.style.left = `${(hero.position.x / 64) * 100}%`;
    blip.style.top = `${(hero.position.z / 64) * 100}%`;
  }
